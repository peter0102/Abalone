...
int areOpponentsNear(Plateau p){ // compte le nombre d'ennemis proches (à 1,2 et 3 cases) et retourne un score
    ...
    for(int i=1;i<MAX_I-1;i++) {
        for (int j=1;j<MAX_J-1;j++) {
            if (p[i][j]==CASE_NOIRE) {
                if (p[i+1][j]==CASE_BLANCHE) score+=-5; // -5 si adversaire au contact, -3 si à une case distante, -1 si à 2 cases distantes
                if (p[i][j+1]==CASE_BLANCHE) score+=-5;
                if (p[i+2][j]==CASE_BLANCHE) score+=-3;
                if (p[i][j+2]==CASE_BLANCHE) score+=-3;
                if (p[i+3][j]==CASE_BLANCHE) score+=-1;
                if (p[i][j+3]==CASE_BLANCHE) score+=-1;
                if (p[i-1][j]==CASE_BLANCHE) score+=-5;
                if (p[i][j-1]==CASE_BLANCHE) score+=-5;
                if (p[i-2][j]==CASE_BLANCHE) score+=-3;
                if (p[i][j-2]==CASE_BLANCHE) score+=-3;
                if (p[i-3][j]==CASE_BLANCHE) score+=-1;
...
int miniMax(Plateau p,int depth,int alpha,int beta,bool isMaximizingPlayer){
	...
	if(isMaximizingPlayer){ 
        ...
        for (int i=0;i<lengthOfMoves;i++) {
        	...
        	m[0][1]=charac[3]-'@';
        	m[1][1]=charac[4]-'0';
            allMove(p,m,CASE_NOIRE,CASE_BLANCHE); // effectue un mouvement
...
char aiMove(Plateau p){
    ...
    for(int i=0;i<lengthOfMoves;i++){
        ...
        m[0][1]=charac[3]-'@';
        m[1][1]=charac[4]-'0';
        if (p[m[0][1]][m[1][1]]!=CASE_NOIRE) {
        allMove(p,m,CASE_NOIRE,CASE_BLANCHE);
        int newScore=miniMax(p,2,-INFINITY,INFINITY,false);
