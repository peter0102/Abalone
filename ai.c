...
int miniMax(Board board,int depth,int alpha,int beta,bool isMaximizingPlayer, char current_player, char other_player){
	...
	if(depth==0){ //S'arrête lorsque la profondeur souhaitée est atteinte
        ...
        int score=-INFTY;
        lmove(board,BLACK,list);
        for (int i=0;i<56;i++) {
            allMove(board,list[i],BLACK,WHITE); // effectue un mouvement
        ...
        int score=INFTY;
        lmove(board,WHITE,list);
        for (int i=0;i<56;i++) {
            allMove(board,list[i],WHITE,BLACK);
...
void aiBestMove(Move move, Board board, char current_player, char other_player){
    ...
    lmove(board,BLACK,list);
    for(int i=0;i<56;i++){
        allMove(board,list[i],BLACK,WHITE);
        display(board);
